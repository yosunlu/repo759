...
void getAcc(const double pos[][3], const double mass[], double acc[][3], int N) {
    ...
    for (int i = 0; i < N; i++) {
        acc[i][0] = acc[i][1] = acc[i][2] = 0.0; // Reset acceleration for particle i
        for (int j = 0; j < N; j++) {
            if (i != j) {
...
int main(int argc, char *argv[]) {
    ...
    int num_threads = std::stoi(argv[3]); // Number of OpenMP threads
    ...
    for (int step = 0; step < Nt; step++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            pos[i][0] = std::fmax(std::fmin(pos[i][0], board_size), -board_size);
            pos[i][1] = std::fmax(std::fmin(pos[i][1], board_size), -board_size);
            pos[i][2] = std::fmax(std::fmin(pos[i][2], board_size), -board_size);
        }
